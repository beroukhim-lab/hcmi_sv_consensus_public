...
#New code from Sean to read in named arguments
while getopts "a:o:i:n:" var
do  
    case "$var" in
        a) aliquot_id=${OPTARG};; #Input a single value for the aliquot id
        o) output_directory=${OPTARG};; #Input a single directory to output the files. The directory must exist.
        i) input_array=${OPTARG};; #Input an array of paths to bedpe files. This argument can accept any number of bedpe files.
        n) caller_name_array=${OPTARG};; #Input an array of caller names. The caller names can be whatever you want, but the length must match length of the -i argument.
    esac
done

#Make a log file
touch $aliquot_id.log
echo "log file for running sv_consensus on $aliquot_id" >> $output_directory/$aliquot_id.log #Add the sample name to the log file
echo date +"%y%m%d" >> $output_directory/$aliquot_id.log #Add the date to the log file


#Echo the information and add to the log file
echo "Files will be output to $output_directory" | tee -a $output_directory/$aliquot_id.log #Add the output directory to the log file

#Print all of the input paths and add to the log file
i=1
for file in $input_array; do
    echo "Bedpe file number $i in the array is $file" | tee -a $output_directory/$aliquot_id.log 
    i=i+1
done
...
for caller in $caller_name_array; do
    echo "Caller number $i is named $caller" | tee -a $output_directory/$aliquot_id.log
    i=i+1
done
...
# Argument parsing
aliquot_id = $1 # Argument 1: sample name
output_directory = $2 # Argument 2: output directory
shift 2 # Shift all arguments to the left (original $1 and $2 gets lost)
bedpe_files = ("$@") # Argument 3: bedpe file paths
...
today = $(date +"%y%m%d") # Save today's date as a variable
CODE_DIR = "/opt/scripts" # Where all scripts are stored
...
# TODO: Logging: Not really necessary but could be helpful to add? TBD
echo -en "" > ${log}
...
SV_MASTER=${SAMPLE_DIR}/masterSvList.bed # Master file path
